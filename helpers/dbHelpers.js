module.exports = (knex) => {
	...
	const getCategories = (user_id) => {
		console.log(user_id);
		return knex.select('*').from('categories').where({ user_id: user_id });
	};
	...
	const getAppointments = (user_id) => {
		return knex.select('*').from('appointments').where({ user_id: user_id });
	};
	...
	const deleteCategory = (category) => {
		return knex('categories').where({ id: category.id }).del();
	};
	...
	const deleteNote = (note) => {
		return knex('notes').where({ id: note.id }).del();
	};
	...
	const updateAppointment = (appointment) => {
		return knex('appointments').where({ id: appointment.id }).update({
	...
	const deleteAppointment = (appointment) => {
		return knex('appointments').where({ id: appointment.id }).del();
	};
